...
from aiocaldav import error
from aiocaldav.lib.aio import AsyncInit
...
class TestUser(metaclass=AsyncInit):
    async def __init__(self, i):
...
async def main():
    ...
    attendees.append(await organizer.principal.get_vcal_address())
    ...
    all_cnt = 0
    invite_req_cnt = 0
    for inbox_item in await attendee1.principal.schedule_inbox().get_items():
        all_cnt += 1
    ...
    for inbox_item in await organizer.principal.schedule_inbox().get_items():
